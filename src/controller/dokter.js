...
class DokterController {
    ...
    static getAllDokter (req, res, next){
        Dokter.find()
        .then(dokters => {
            let doktersBaru = dokters.map(dokter => {
                return {
                    fullName: dokter.fullName,
                    jamPraktek: dokter.jamPraktek,
                    statusPraktek: dokter.statusPraktek,
    ...
    static refresh (req, res, next){
        ...
        .then(dokter => {
            if(dokter){
                const payload = generateJWTToken({_id: dokter._id, username: dokter.username})
                res.status(200).json({fullName: dokter.fullName, token: payload, statusPraktek: dokter.statusPraktek})
            }else {
